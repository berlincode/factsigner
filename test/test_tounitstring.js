  ...
  describe('toUnitString', function() {
    ...
    it('examples ndigit=2', function(done) {
      ...
      assert.equal( // no flooring/rounding/ceiling - just stripping digits: 0.018 -> 0.01
        factsigner.toUnitString(web3_utils.toBN('0x3ff2e795f50000'), 18, 2), // (10**15) *18
        '0.01'
      );

      assert.equal( // no flooring/rounding/ceiling - just stripping digits: -0.018 -> -0.01
        factsigner.toUnitString(web3_utils.toBN('0x3ff2e795f50000').neg(), 18, 2), // - (10**15) *18
        '-0.01'
      );
      done();
    });
    ...
    it('examples ndigit=1', function(done) {
      ...
      assert.equal( // no flooring/rounding/ceiling - just stripping digits: 0.18 -> 0.1
        factsigner.toUnitString(web3_utils.toBN('0x27f7d0bdb920000'), 18, 1), // (10**16) *18
        '0.1'
      );

      assert.equal( // no flooring/rounding/ceiling - just stripping digits: -0.18 -> -0.1
        factsigner.toUnitString(web3_utils.toBN('0x27f7d0bdb920000').neg(), 18, 1), // - (10**16) *18
        '-0.1'
      );
      done();
    });
    ...
    it('examples ndigit=0', function(done) {
      ...
      assert.equal( // no flooring/rounding/ceiling - just stripping digits: 21.8 -> 21
        factsigner.toUnitString(web3_utils.toBN('0x12e89287fa7840000'), 18, 0), // (10**17) *218
        '21'
      );

      assert.equal( // no flooring/rounding/ceiling - just stripping digits -21.8 -> -21
        factsigner.toUnitString(web3_utils.toBN('0x12e89287fa7840000').neg(), 18, 0), // - (10**17) *218
        '-21'
      );
    ...
    it('examples ndigit=-1', function(done) {
      ...
      assert.equal(
        factsigner.toUnitString(web3_utils.toBN('0x18493fba64ef00000'), 18, -1), // (10**18) *28
        '20'
      );

      // no flooring/rounding/ceiling - just stripping digits -28 -> -20
      assert.equal(
        factsigner.toUnitString(web3_utils.toBN('0x18493fba64ef00000').neg(), 18, -1), // - (10**18) *28
        '-20'
      );
